import { useGoals, useUpdateGoal, useGoalFeedbacks } from "@/hooks/use-career-data";
import { formatDate, statusColors } from "@/lib/utils";
import { Table, TableBody, TableCaption, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Edit, Eye, Send, CheckCircle2, XCircle, Plus, MessageSquare, CornerDownRight, AlertTriangle } from "lucide-react";
...
export function GoalsTable({ timeFilter = "all", filterByTimePeriod }: GoalsTableProps) {
  ...
  if (isError) {
    return (
      <div className="bg-white rounded-lg overflow-hidden">
        <div className="p-6">
          <p className="text-red-500">There was an error loading your goals. Please try again later.</p>
        </div>
      </div>
    );
  }
    ...
    try {
      await updateGoalMutation.mutateAsync({
        ...
        data: { 
          ...
          progress: statusToUpdate === "completed" ? 100 : 
                   statusToUpdate === "in_progress" ? 50 : 
                   statusToUpdate === "pending_review" ? 90 : 
                   undefined
        }
      });
      toast({
        title: "Goal updated",
        description: `Goal status updated to ${statusToUpdate.replace('_', ' ')}`,
      });
  ...
  const updateGoalStatusDirectly = async (goalId: number, newStatus: GoalStatus) => {
    try {
      await updateGoalMutation.mutateAsync({
        id: goalId,
        data: { 
  ...
  const renderStatusBadge = (status: string) => {
    ...
    let label = status.replace(/_/g, ' ').split(' ').map(word => 
    ...
    switch (goal.status) {
      case "draft":
        actions.push(
          <Button 
            ...
            onClick={() => updateGoalStatusDirectly(goal.id, "pending_confirmed")}
        ...
        if (isEmployee) {
          ...
          actions.push(
            <Button 
              ...
              variant="default"
              className="ml-2 min-w-[150px] bg-blue-600 hover:bg-blue-700 text-white"
              onClick={() => updateGoalStatusDirectly(goal.id, "pending_review")}
              disabled={updateGoalMutation.isPending}
            >
      ...
      case "review": // For compatibility with old data
        if (isEmployee) {
          ...
          actions.push(
            <span className="text-xs text-neutral-500 ml-2">Awaiting Review</span>
          );
        } else {
          ...
          <div className="flex items-start space-x-2 p-2 rounded-md">
            ...
            <div className="flex-1">
              <div className="font-medium text-sm text-amber-800 flex items-center">
                ...
                This goal was sent back from {sortedFeedbacks[0]?.feedbackGoalStatus || "Review"}
              </div>
              <div className="mt-2 space-y-3">
                {sortedFeedbacks.map((feedback) => (
                  <div key={feedback.id} className="bg-white p-3 rounded-md shadow-sm border border-amber-200">
                    ...
                    <p className="text-sm text-neutral-700 whitespace-pre-line">{feedback.content}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div>
        <Table className="rounded-lg overflow-hidden">
          ...
          <TableHeader className="bg-blue-600 border-b border-blue-500">
            <TableRow>
              <TableHead className="w-[200px] font-semibold text-white">Goal</TableHead>
              <TableHead className="w-[300px] font-semibold text-white">Description</TableHead>
              <TableHead className="font-semibold text-white">Category</TableHead>
              <TableHead className="font-semibold text-white">Delegator</TableHead>
              <TableHead className="font-semibold text-white">Start Date</TableHead>
              <TableHead className="font-semibold text-white">Target Date</TableHead>
          ...
          <TableBody>
            {goals && goals
              .filter(goal => {
                ...
                } else if (["in_progress", "pending_review", "reviewed"].includes(goal.status)) {
                  ...
                  <>{/* Using fragment shorthand instead of React.Fragment */}
                    <TableRow key={goal.id}>
                      ...
                      <TableCell className="max-w-xs">
                        <div className="truncate">{goal.description}</div>
                      </TableCell>
                    ...
                    {(goal.status === "in_progress" || goal.status === "draft") && <FeedbackRow key={`feedback-${goal.id}`} goalId={goal.id} />}
                  </>
                );
              })}
          </TableBody>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Change the status of this goal. This will update the goal's progress and visibility.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="py-4">
              ...
              <Select 
                ...
                onValueChange={(value) => setStatusToUpdate(value as GoalStatus)}
              >
                <SelectTrigger>
        ...
        <Dialog open={isEditGoalDialogOpen} onOpenChange={setIsEditGoalDialogOpen}>
          <DialogContent className="sm:max-w-[625px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Update the progress of this goal and add a comment about your progress.
              </DialogDescription>
            </DialogHeader>
            {goalToEdit && (
              <EditGoalForm 
